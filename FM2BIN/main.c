...
static char fileTypeJPG[25]="‘¦ â­®¥ JPEG ¨§®¡à ¦¥­¨¥\0";
static char fileTypePNGcaps[3]="PNG";
static char fileTypePNGnocp[3]="png";
static char fileTypePNG[34]="‘¦ â®¥ ¡¥§ ¯®â¥àì PNG ¨§®¡à ¦¥­¨¥\0";
...
int main(int argc, char* argv[])
{
	...
	if(crclib==NULL)
	{
		printf("¥¢®§¬®¦­® ®âªàëâì ¬®¤ã«ì CRCLIB.DLL\nŠ®­âà®«ì­ë¥ áã¬¬ë ¯à®¢¥àïâìáï ­¥ ¡ã¤ãâ!\n\n");
	}
	else
	{
		function libver = (function)GetProcAddress(crclib,"libVer");
		printf("CRCLIB.DLL ­ ©¤¥­ ¨ ®âªàëâ\n");
		printf("livBer");
		printf("\n\n");
		crclibLoad=1;
	}
	...
	{
		//printf("‡ ­¥á¥­¨¥ §­ ç¥­¨ï bufferPos ¢ áâ¥ª\n");
		dataStack(1,bufferPos);
		//printf("“áâ ­®¢ª  ªãàá®à  ¢ ¯®â®ª¥ FIn ¯®  ¤à¥áã %d\n",fileIndex);
		fseek(FIn,fileIndex,SEEK_SET);
		//printf("—â¥­¨¥ ¤ ­­ëå ¢ ¡ãä¥à\n");
		readResult=fread(inputBuffer,1,bufferLength,FIn);
		//printf("readResult=%d ¯à®å®¤=%d fileIndex=%d bufferPos=%d\n",readResult,steps,fileIndex,bufferPos);
		//printf(" ç «® æ¨ª«  ¤¥ª®¤¨à®¢ ­¨ï áç¨â ­­®£® ¡«®ª \n");
		bufferPos=0;
		while(1)
		{
			//printf("à®¢¥àª  ä« £  startLock(%d)\n",startLock);
			if(startLock==0)
			{
				cacheBufferPos=bufferPos;
			}
			//printf("‚ë§®¢ äã­ªæ¨¨ ¤¥ª®¤¥à  ¯ ª¥â®¢(ãª § â¥«ì bufferPos=%d\n",bufferPos);
			bufferPos=packetConverter(inputBuffer, bufferPos, bufferLength);
			//printf("‡ ¯¨áì ¤¥ª®¤¨à®¢ ­­®£® ¡ ©â  0x%02X ¢ ¬ áá¨¢ ¯®  ¤à¥áã %d\n",resultByte,steps);
			procBuf[decodeBufIndx]=resultByte;
			if(bufferPos>bufferLength)
			{
				printf("‚­¨¬ ­¨¥! ‚ëå®¤ §  ¯à¥¤¥«ë ¬ áá¨¢ (%d>%d)\n",bufferPos,bufferLength);
			}
		...
		//printf("Š®­âà®«ì­ ï áã¬¬  CRC-16 ¡«®ª : 0x%04X\n",decFileCRC16);
		memset(valBuffer,0x00,sizeof(valBuffer));
		//printf("„¥ª®¤¨à®¢ ­¨¥... ¯à®å®¤ %d, ¨áå®¤­ë© ç ­ª %d(à §¬¥à ç ­ª  %d), ª®­¥ç­ë© ç ­ª %d, CRC-16: 0x%04X\n",steps+1,fileIndex,dataStack(0,0),fileWriteIndex,decFileCRC16);
		if(steps==numberOfIterations-1)
		{
			//printf("‡ ¯¨áì ®áâ âª  à §¬¥à®¬ %d ¡ ©â... ",decFileDecLtSz+1);
...
uint32_t measurePulse(uint8_t *samplesArr)
{
	...
	pulseCnt=0;
	trigPause=0;
	if(((mFlags&0x20)==0)&&(mFlags&0x10)==0)	//ˆ§¬¥àï¥¬ ¨¬¯ã«ìáë «®£.1.  ã§ã â®¦¥ á¬®âà¨¬
	{
		_j=0;
		while(1)
		{
	...
	pulseCnt=0;
	trigPause=0;
	if(((mFlags&0x08)==0)&&(mFlags&0x04)==0)	//ˆ§¬¥àï¥¬ ¨¬¯ã«ìáë «®£.0. ’ ª¦¥ ¬¥àï¥¬ ¯ ã§ã
	{
		_j=0;
		while(1)
		{
	...
	printf(" §¬¥à ¡ãää¥à  ¤«ï çâ¥­¨ï: %d  ©â\n",newBufferLength);
	printf(" §¬¥à ª «¨¡à®¢®ç­®£® ¯®«ï: %d\n",_k-44);
	bufferLength=newBufferLength;
	inputBuffer=realloc(inputBuffer,newBufferLength);
	if(inputBuffer==0)
	{
		printf("Žè¨¡ª  ­ áâà®©ª¨ ¡ãä¥à (æ¥«¥¢®¥ §­ ç¥­¨¥=%d ¡ ©â)\n",newBufferLength);
	}
	return _k;
}
uint32_t packetConverter(uint8_t *sourceArray, uint32_t arrPos, uint32_t bufferSize)
