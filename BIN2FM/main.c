...
int main(int argc, char* argv[])
{
	...
	bufferMas = malloc(bufferLength);				//¡ãä¥à ¯®¤ ¯ çªã á¥¬¯«®¢ £¥­¥à¨àã¥¬ëå ¨§ 256 ¡ ©â ¤ ­­ëå. „®«¦­® å¢ â¨âì
	...
	if(codePageNum!=866){
		...
		{printf(codePageInfo);}}
	else{printf("¥à¥ª«îç¥­¨¥ ª®¤®¢®© áâà ­¨æë ­¥ âà¥¡ã¥âáï\n");}
	printf("à¥®¡à §®¢ â¥«ì ä®à¬ â®¢ BIN->WAV(¤¨áªà¥â­ ï FM ¬®¤ã«ïæ¨ï) \n");
	printf(StartMsg);
	printf("ã ç® ¯ æ ­ë, ï á«¥â¥« á è à £¨ â ª çâ® â¥¯¥àì ¯à®£  ¡ëáâà¥¥ ¡ã¤¥â ¯¨á âìáï. \n‚á¥¬ £ã«ï¬ ªã ®áâ «ì­ë¬ á®¡®«¥§­ãî\n");
	FIn=fopen(argv[1],"rb");
	if(!FIn&&argc<3){
	...
	while(writeIterations<numberOfIterations+2){
		...
		if(writeIterations>numberOfIterations){
			printf("‡ ¯¨áì ®áâ â®ç­®£® ¡«®ª  ü%d à §¬¥à®¬ ¢ %d ¡ ©â\n",writeIterations,roundedVal);
	...
	printf("‘ã¬¬ à­ë© à §¬¥à § ¯¨á ­­®£® ä ©« : %d ¡ ©â\n",transferIndex);
	printf("” ©« ­ áâà®¥ª convertParams.ini ãá¯¥è­® á®§¤ ­\n’¥¯¥àì ­¥®¡å®¤¨¬® § ¯ãáâ¨âì WAVBIND.EXE, çâ®¡ë áä®à¬¨à®¢ âì WAV ä ©«\n");
	free(bufferMas);
	fclose(FIn);
	fclose(FOut);
	storeStructToFile();
